...
const features = [
  {
    ...
    description:
      "Detects dead-end connections, suggests high-value alternatives, and automatically maintains relationship health.",
    icon: RefreshCw,
  },
  {
    title: "On-demand Pipelines",
    description:
      "Generate tailored reachout sequences and relationship workflows on your own infrastructure.",
    icon: Zap,
  },
  {
    title: "Agentic Nurturing",
    description:
      "Autonomous agents handle the initial friction of professional networking, so you only join when it's high value.",
    icon: Layers,
  },
  {
    title: "Trust & Observability",
    description:
      "Manage your professional brand across all platforms in one centralized, agent-powered location.",
    icon: Shield,
  },
];
    ...
    <section className="py-40 bg-white/[0.01] border-y border-white/5">
      ...
      <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-white/5 border border-white/5 rounded-[2.5rem] overflow-hidden">
        {features.map((f, i) => (
          <div
            key={i}
            className="p-10 bg-background hover:bg-white/[0.02] transition-colors group"
          >
